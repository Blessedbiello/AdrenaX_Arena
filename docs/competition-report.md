...
A simulated test competition was run to validate the full lifecycle of AdrenaX Arena features. The test covered duel creation, acceptance, settlement, streak tracking, open challenges, revenge mechanics, and spectator predictions.
...
**Result:** Duel created with pending status, challenge URL and card URL returned. Challenge card renders as a 1200x630 PNG with Inter font, showing challenger/defender addresses, asset, duration, and "DO YOU ACCEPT?" CTA.
...
3. Determines winner by higher total ROI
...
6. Calls `updateStreaks()` — winner's streak increments, loser's resets
7. Creates Mutagen reward (50 base * streak multiplier)
8. Creates revenge window in Redis (30-min TTL) for the loser
9. Settles predictions (correct = 10 MUTAGEN, incorrect = 0)
...
6. **Clan Wars implementation** — Team-based competition with synergy bonuses (spec complete in design.md)
7. **Season Pass with milestones** — Progressive rewards for cumulative achievements
8. **On-chain escrow** — Replace custodial stake handling with Anchor program for trustless staked duels
9. **Adrena Quest integration** — Wire arena events to Adrena's quest system for cross-feature engagement
10. **Tournament brackets** — Visual bracket display for Gauntlet rounds
